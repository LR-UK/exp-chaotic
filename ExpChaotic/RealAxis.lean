...
theorem lemma6_accumulation_up
  ...
  · intro z hz
    ...
    have hclose : ‖z - Y (φ k)‖ < δ0 := by
      ...
      calc ‖z - Y (φ k)‖ = ‖(z - ystar) + (ystar - Y (φ k))‖ := by ring_nf
        _ ≤ ‖z - ystar‖ + ‖ystar - Y (φ k)‖ := norm_add_le _ _
        _ < δ0 := by
            rw [← norm_neg (ystar - Y (φ k))] at *
            simp only [neg_sub] at *
            linarith
